        ...
        <div className="overflow-x-auto">
          <Table striped>
            ...
            <Table.Body className="divide-y">
              <Table.Row
                className={`${
                  dataFindProcces == undefined || dataFindProcces.length === 0
                    ? ``
                    : `hidden`
                }`}
              >
                <Table.Cell colSpan={3}>
                  <p className={`text-center`}>
                    Tidak ada Activitas yang dipilih
                  </p>
                </Table.Cell>
              </Table.Row>
              {dataFindProcces &&
                dataFindProcces.map((item, index) => (
                  <Table.Row
                    key={index}
                  ...
                  >
                    ...
                    <Table.Cell onClick={() => handleDelete(index)}>
                      {" "}
                      {/* Panggil handleDelete dengan index */}
                      <svg
              ...
              <Table.Row>
                <Table.Cell
                  onClick={handleAdd}
                  colSpan={3}
                  className={`hover:bg-gray-100 bg-gray-50 p-2 ${
                    dataFindProcces == undefined || dataFindProcces.length === 0
                      ? `hidden`
                      : ``
                  }`}
