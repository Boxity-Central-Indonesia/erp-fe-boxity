...
export const GoodReceiptDetail = ({data, defaultEdit, handleEdit, dataHeading}) => {
    ...
    return(
        <>
        <h1 className="text-2xl my-5 dark:text-white font-semibold">Good receipt detail</h1>
        <section className="bg-white dark:bg-gray-800 dar00k:text-white rounded-md shadow-md p-5 mb-16">
            <div className="grid lg:grid-cols-2 text-base">
                <div className="col-span-1">
                    <table className={`w-full`}>
                        <tr className="">
                            <td className="py-3">Receipt code</td>
                            <td> : <span className="ml-5">{data?.kodeGoodsReceipt || '--'}</span></td>
                        </tr>
                        <tr className="">
                            <td className="py-3">Order code</td>
                            <td> : <span className="ml-5">{data?.order.kode_order || '--'}</span></td>
                        </tr>
                        <tr>
                            <td className="py-3">Receipt date</td>
                            <td> : <span className="ml-5">{data?.created_at || '--'}</span></td>
                        </tr>
                        <tr>
                            <td className="py-3">Purchase date</td>
                            <td> : <span className="ml-5">{data?.order.created_at || '--'}</span></td>
                        </tr>
                    </table>
                </div>
                <div className="col-span-1">
                    <table className="w-full">
                        <tr>
                            ...
                            <td> : <span className="ml-5">{data?.status || '--'}</span></td>
                        </tr>
                        ...
                        <tr>
                            ...
                            <td> : <span className="ml-5">{data?.details || '--'}</span></td>
                        </tr>
                    </table>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 gap-5">
               <div>
               <h2 className="text-xl font-medium dark:text-white mb-4">Good receipt item</h2>
               <TabelForDetail data={dataGoodReceiptItem} dataHeading={dataHeading} handleEdit={handleEdit} routes={'products'}/>
               </div>
            </div>
            <hr className="my-7" />
        </section>
