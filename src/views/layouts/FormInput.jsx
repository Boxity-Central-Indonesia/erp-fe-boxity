...
export const TextArea = ({span, label, htmlFor, onChange, name,id, value, referens,  validationError, placeholder, rows}) => {
    return(
        <>
             <div className={span}>
                    <label htmlFor={htmlFor} className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{label}</label>
                    <textarea value={value} onChange={onChange} name={name} ref={referens} id={id} rows={`${rows ?? `4`}`} className="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder={placeholder}></textarea>
                    <p className={`text-red-500 mt-2 text-sm ${!!validationError && validationError[name] ? '' : 'hidden'}`}>
                    {!!validationError && validationError[name] ? validationError[name] : ''}
                  </p>
                </div>
