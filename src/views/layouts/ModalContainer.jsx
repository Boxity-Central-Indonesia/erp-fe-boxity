...
export const ModalConfirmDelete = ({handleDelete, modalDelete, closeModalDelete}, ) => {
  return(
   <>
    <div id="deleteModal" tabIndex="-1" aria-hidden="true" className={`${modalDelete ? 'flex' : 'hidden'} overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-modal md:h-full`}>
        <div className="relative p-4 w-full max-w-md h-full md:h-auto">
            <div className="relative p-4 text-center bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                <button onClick={closeModalDelete} type="button" className="text-gray-400 absolute top-2.5 right-2.5 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-toggle="deleteModal">
    ...
    <div modal-backdrop="" className={`${modalDelete? 'flex' : 'hidden'}  bg-gray-900 bg-opacity-50 dark:bg-opacity-80 fixed inset-0 z-40`}></div>
   </>
  )
}