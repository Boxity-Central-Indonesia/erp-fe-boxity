...
const TabelHeading = ({ toggleOpenModal, icon, label, globalFilter, setGlobalFilter }) => {
  ...
  return(
    <div className="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
        ...
        <form className="flex items-center">
          ...
          <div className="relative w-full">
            ...
            <DebouncedInput value={globalFilter ?? '' } onChange={value=> setGlobalFilter(String(value))}
              type="text" id="simple-search" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm
      ...
      <div
        ...
        < Button event={()=> toggleOpenModal()} label={label} icon={icon} type={'button'} bgColour={'primary'}
          paddingY={'2.5'}/>
          <DropdownComponent />
...
const DropdownComponent = () => {
  return (
    <div className='border border-gray-400 py-2 dark:text-white text-sm px-4 rounded-md'>
      <Dropdown dismissOnClick={false} label="Employment status" size={'lg'} inline className=''>
      <Dropdown.Item>
        Home
      </Dropdown.Item>
      <Dropdown.Item>
...
function DebouncedInput({
  ...
  return <input {...props} value={value} onChange={e => setValue(e.target.value)} autoFocus/>;
}
