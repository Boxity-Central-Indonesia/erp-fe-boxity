...
const TabelHeading = ({
  ...
  return (
    <>
      <section>
        <div
          className={`flex relative mb-8 mt-3 ${
        ...
        >
          ...
          <button
            className={`btn_akses px-8 pb-3 ${
              activeButton === "employee-categories"
                ? "border-b-4 border-[#f95b12] z-10"
                : "border-b-2"
            }`}
            onClick={() => handleClick("employee-categories")}
          >
            Employes category
          </button>
        ...
        <div className="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
          ...
          <div className="w-full md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0">
            <Button
              event={() => toggleOpenModal(activeButton)}
              label={label}
              icon={icon}
              type={"button"}
              bgColour={"primary"}
